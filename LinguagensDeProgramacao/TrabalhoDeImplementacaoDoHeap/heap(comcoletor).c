...
int main(){
    ...
    gets(nomeDoArquivo);
    system("cls");
    printf("\n<0> Sem coletor de lixo\n<1> Com coletor de lixo\n");
    scanf("%d", &COLETOR);
    ...
    if(!resp)
    {
        ...
        if(e_varInicioDeBloco != sofreuDel) // Se variável da esquerda não foi desalocada antes da atribuição, a área para a qual ela apontava vira lixo
            if(!COLETOR)
                ...
                AtualizarHeap(heap, e_varInicioDeBloco, e_varBlocosContiguos, livre);
                InserirAreaLivre(listaAreaLivre, e_varInicioDeBloco, e_varBlocosContiguos);
